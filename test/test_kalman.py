...
def test_kalman_predict():
  ...
  """
  state = GaussianState(mean=torch.Tensor([11.35, 4.5]),
                        covar=torch.Tensor([[545, 150], [150, 500]]))
  dt = 0.3
  ...
  x_expected, P_expected = predict(
      x=state[0]['mean'], P=state[0]['covar'], F=F, Q=Q)
...
def test_kalman_update():
  ...
  z = 1
  state = GaussianState(mean=torch.Tensor([12.7, 4.5]),
                        covar=torch.Tensor([[545, 150], [150, 500]]))
  # state = torch.cat((state, state.clone()))
  kf = KalmanFilter(
      transition_model=TestLinearTransitionModel(),
      measurement_model=TestLinearMeasurementModel(),
  )
  ...
  x_expected, P_expected = update(
      x=state[0]['mean'], P=state[0]['covar'], z=z, R=R, H=H)
