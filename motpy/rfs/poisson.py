...
class Poisson:
  ...
  def __init__(
    ...
    ndim_state = birth_states['mean'].shape[-1]
    self.weights = torch.tensor([], dtype=torch.float32)
    self.states = GaussianState(
        mean=torch.zeros(0, ndim_state, dtype=torch.float32),
        covar=torch.zeros(0, ndim_state, ndim_state, dtype=torch.float32))

  def __repr__(self):
    return f"""PoissonPointProcess(
  ...
  def predict(self,
    ...
    pred_ppp.weights = torch.cat((self.weights*ps, self.birth_weights))
    pred_ppp.states = torch.cat((
        state_estimator.predict(state=self.states, dt=dt),
        self.birth_states))
